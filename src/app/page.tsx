				...
				<div className="mt-6 w-full border border-border-primary">
					...
					{leaderboardData.map((row, i) => (
						...
						>
							<span
								className={[
									"w-[50px] font-mono text-sm font-bold",
									rankClasses[row.rank] ?? "text-text-secondary",
								].join(" ")}
							>
								{row.rank}
							</span>
							<span className="w-[70px] font-mono text-sm font-bold text-accent-red">
								{row.score.toFixed(1)}
							</span>
							<pre className="flex-1 font-mono text-xs leading-relaxed text-text-secondary">
								{row.code}
							</pre>
							<span className="w-[100px] text-right font-mono text-xs text-text-tertiary">
								{row.lang}
							</span>
						</div>
					))}
